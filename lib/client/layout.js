...
BlazeLayout.render = function render(layout, regions, keepExisting) {
  regions = regions || {};
  Meteor.startup(function() {
    ...
    if(!_isReady) {
      Meteor.defer(function() {
        _isReady = true;
        BlazeLayout._render(layout, regions, keepExisting)
      });
    } else {
      BlazeLayout._render(layout, regions, keepExisting);
    }
...
BlazeLayout._removeRegions = function _removeRegions(regions) {
...
BlazeLayout._updateRegions = function _updateRegions(regions, keepExisting) {
  var needsRerender = false;
  if(!keepExisting) BlazeLayout._removeRegions(regions);
...
BlazeLayout._render = function _render(layout, regions, keepExisting) {
  ...
  if(currentLayoutName != layout) {
    ...
    BlazeLayout._updateRegions(regions, keepExisting);
  }
};
