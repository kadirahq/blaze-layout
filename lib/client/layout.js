...
BlazeLayout.render = function render(layout, regions, options) {
  regions = regions || {};
  options = options || {};
  Meteor.startup(function() {
    ...
    if(!_isReady) {
      Meteor.defer(function() {
        _isReady = true;
        BlazeLayout._render(layout, regions, options);
      });
    } else {
      BlazeLayout._render(layout, regions, options);
    }
...
BlazeLayout._updateRegions = function _updateRegions(regions, options) {
  var needsRerender = false || options.force;
...
BlazeLayout._render = function _render(layout, regions, options) {
  ...
  if(currentLayoutName != layout) {
    ...
    BlazeLayout._updateRegions(regions, options);
  }
};